...
static int16_t ECOCALLMETHOD CEcoMemoryManager1Lab_623E1838_Init(/* in */ IEcoMemoryManager1Ptr_t me, /* in */ voidptr_t startAddress, /* in */ uint32_t size) {
    CEcoMemoryManager1Lab_623E1838* pCMe = (CEcoMemoryManager1Lab_623E1838*)me;
    MemoryHeader* pFirstHeader = 0;
    /* Проверка указателей */
    if (me == 0) {
        return -1;
    }
    
    /* Инициализация указателей кучи */
    pCMe->m_pHeapStart = (char_t*)startAddress;
    pCMe->m_pHeapEnd = (char_t*)startAddress + size;
    pCMe->m_iHeapSize = size;
    pCMe->m_iAllocSize = 0;
    pCMe->m_iBlockNum = 0;
    ...
    /* Создаем первый заголовок, который описывает всю память как один свободный блок */
    /* Проверяем, достаточно ли места для хотя бы одного заголовка */
    if (size < sizeof(MemoryHeader)) {
        return -1;
    }

    pFirstHeader = (MemoryHeader*)startAddress;
